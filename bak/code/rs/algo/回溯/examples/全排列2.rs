...
fn backtracing(
    ...
    for i in start_index..nums.len() {
        // 数组中有两个连续的点，如果这第一次被采过，就会进入递归，第二次应该继续采
        // 同一树枝可以重复选择，[树枝上去重](https://img-blog.csdnimg.cn/20201124201406192.png)
        // if i > start_index && nums[i] == nums[i - 1] && skip_indexs[i - 1] == false {
        //     continue;
        // }
        // 这种也是对的，[树层上去重](https://img-blog.csdnimg.cn/20201124201431571.png)
        if i > start_index && nums[i] == nums[i - 1] && skip_indexs[i - 1] == true {
            continue;
        }
        // 当前点被采过用过
        if skip_indexs[i] {
